...
<body class="bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-200 antialiased font-sans transition-colors duration-500 selection:bg-violet-500/30">
    ...
    {{-- NAVBAR --}}
    <nav class="fixed top-0 inset-x-0 z-50 backdrop-blur-xl bg-white/80 border-b border-slate-200 dark:bg-slate-900/80 dark:border-slate-800 transition-colors duration-300">
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="{{ route('home') }}" class="font-serif italic text-xl text-violet-500 dark:text-violet border-l border-slate-300 dark:border-slate-700 pl-4 hidden sm:block hover:text-violet-600 dark:hover:text-violet-light transition-colors">
                ✦ Voting Karya
            </a>
        </div>
            <div class="flex items-center gap-2 sm:gap-4">
                ...
                @auth
                    <span class="text-sm text-slate-600 dark:text-slate-400 hidden sm:inline">{{ auth()->user()->name }}</span>
                ...
                @else
                    ...
                    <a href="{{ route('register') }}" class="text-sm px-5 py-2.5 rounded-full bg-violet-500 text-white dark:bg-violet-600 font-semibold hover:bg-violet-600 dark:hover:bg-violet-500 transition shadow-sm">Daftar</a>
                @endauth
            </div>
        </div>
    </nav>
