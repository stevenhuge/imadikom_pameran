...
<div class="p-6 max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-semibold text-slate-800 dark:text-white">Tambah Kompetisi Baru</h1>
    </div>

    <form action="{{ route('admin.competitions.store') }}" method="POST" class="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
        @csrf
        
        <div class="mb-4">
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nama Kompetisi</label>
            <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Kompetisi Poster Nasional" class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-violet-500 bg-transparent dark:text-white">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <select name="fee_type" required class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-violet-500 bg-transparent dark:text-white">
                    <option value="free" {{ old('fee_type') == 'free' ? 'selected' : '' }}>Gratis (Free)</option>
                    <option value="paid" {{ old('fee_type') == 'paid' ? 'selected' : '' }}>Berbayar (Paid)</option>
                </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="theme" value="{{ old('theme') }}" placeholder="Tema kompetisi..." class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-violet-500 bg-transparent dark:text-white">
        ...
        <div class="mb-6">
            ...
            <select name="eligibility_type" required class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-violet-500 bg-transparent dark:text-white">
                <option value="1" {{ old('eligibility_type') == 1 ? 'selected' : '' }}>Khusus penerima KIP-K Universitas Amikom Yogyakarta</option>
                <option value="2" {{ old('eligibility_type') == 2 ? 'selected' : '' }}>Khusus Mahasiswa Universitas Amikom Yogyakarta</option>
                <option value="3" {{ old('eligibility_type') == 3 ? 'selected' : '' }}>Khusus penerima KIP-K perguruan tinggi di Indonesia</option>
                <option value="4" {{ old('eligibility_type', 4) == 4 ? 'selected' : '' }}>Seluruh perguruan tinggi yang terdaftar di pemerintah (Nasional/Umum)</option>
            </select>
            @error('eligibility_type') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
        </div>

        <button type="submit" class="px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-sm font-medium transition w-full">Simpan Kompetisi</button>
    </form>
</div>
@endsection
